# Tablet-Only Meeting Room (Meeting SDK Appliance)

Small teams want a TV + external camera + tablet to *be* the whole meeting room — download one
...
- `room/` — the appliance (active), package `com.bilal.meetingsremote`.
...
Appliance (`room/`, package `com.bilal.meetingsremote`):
- `room/build.gradle.kts` — app module; zoomsdk 7.0.5; externalNativeBuild CMake; arm64-only; 16 KB-aligned libs.
- `room/src/main/java/com/bilal/meetingsremote/MainActivity.kt` — dev console UI, adb-scriptable extras, source test.
...
- **Silent no-camera in the 2nd meeting of a process (2026-07-08):** after any `onUninitialized` the SDK forgets the external source, but `setVideoSource()` took a swap-provider shortcut when the source object already existed — the next meeting then had no external camera (no error anywhere; SDK quietly uses the device camera). Fix: call `setExternalVideoSource` on *every* `setVideoSource()`. Lesson: re-register SDK-side state per meeting; our object surviving doesn't mean the SDK's registration did.
...
- **Hosting error 100/80 ROOT-CAUSED (2026-07-08): orphaned PMI, not ZAK.** `leave()` used `leaveCurrentMeeting(false)` — the host *left without ending*, stranding the PMI "in progress" on Zoom's side; a Basic account then can't start its PMI (every attempt fails 100/80) until Zoom reaps the empty meeting (~10 min, observed 16:00→16:10). ZAK was ruled out (backend mints fresh 2 h ZAKs, decoded + verified). Quirks: a *hard-killed* host can rejoin its zombie (start succeeds — but as non-host under a ghost host until heartbeat reaps it); only clean leave-without-end blocks starts. **Fix:** `leave()` ends for all when we're host; `MeetingWatchService` (`stopWithTask=false`, `onTaskRemoved`) ends the meeting when the app is swiped away mid-meeting; error-100 overlay copy now says to wait a minute. Verified: leave → immediate restart INMEETING in 7 s; swipe-away → DISCONNECTING before process death; restart clean (1 participant, no ghosts).
- **Error 100/80 recurrence + auto-recovery (2026-07-09).** It happened again despite the 07-08 fix. Trigger found: `installDebug` at 07:16 hard-killed the app **mid-meeting** (no `onTaskRemoved`) → zombie w/ ghost host; the next Start rejoined it as **non-host**; host migrated ~1 min later when Zoom reaped the ghost; the 07:51 leave (as migrated host) still left the PMI blocked 100/80 for ~10–12 min. Scripted repro (TestHooksReceiver: `leave`/`leaveNoEnd`/`dump` broadcasts + the `startMeeting` extra): 5× rapid start→end-for-all→restart all clean; crash→rejoin→migrated-host end-for-all→restart clean (ghost reaped 52 s after rejoin); 6-min-dwell as migrated host clean; host leave-without-end was still rejoinable at +15 s and +90 s — the blocked state needs ghost-history/longer aging, so the exact morning recipe wasn't fully reproduced. Notable hole: swipe-away while still non-host in a zombie leaves without ending (`isMeetingHost=false` is the true state, not a race). **Fixes:** (1) backend `GET /end-stuck-meeting` force-ends the PMI via REST (`PUT /meetings/{pmi}/status action=end`; needs S2S scope `meeting:update:status:admin` — Zoom code 4711 until it's added in Marketplace); (2) app auto-recovery: Start failing with 100 while hosting → "Recovering the room's meeting…" → backend force-end → one retry (`recoverTried` guard, rearmed per user-initiated start); (3) gradle `endMeetingBeforeInstall` broadcasts a leave before `installDebug` so dev reinstalls can't strand the PMI; (4) `leave()` logs `isMeetingHost/endForAll` for future forensics.

---

## 18. Project History

- **2026-07-09 (error-100 recurrence root-caused + auto-recovery)** — Start Meeting
  failed 100/80 again; forensics + ~25 scripted meeting cycles (details §17): trigger was
  an `installDebug` reinstall mid-meeting (hard kill, no onTaskRemoved). Shipped:
  `TestHooksReceiver` (adb `leave`/`leaveNoEnd`/`dump` broadcasts), leave-decision logging,
  backend `GET /end-stuck-meeting` (REST force-end of the PMI; S2S scope
  `meeting:update:status:admin` added by user), app auto-recovery on error 100
  ("Recovering the room's meeting…" → force-end → one retry), and gradle
  `endMeetingBeforeInstall` so reinstalls end the meeting first. E2E verified on-device:
  force-stop mid-meeting → force-end (204) → restart is a fresh meeting (host immediately,
  no ghost). Zoom-side timings measured: end-for-all → restartable in seconds; hard-kill
  zombie rejoinable (ghost reaped ~1 min after rejoin); unattended reap ~10–12 min.
