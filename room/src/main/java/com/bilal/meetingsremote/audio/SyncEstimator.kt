...
class SyncEstimator(private val onOffset: (ms: Int) -> Unit) {
    ...
    /** Seconds since the camera track last carried real signal ("never" if it
     *  never has) — observability for the sync cascade; a dead/muted mic reads
     *  "never". GCC-PHAT/ML priority is coordinated at apply-time in RoomSdk. */
    fun camSignalAgeS(): String =
        if (lastCamSignalNs == 0L) "never"
    ...
    fun stats(): String =
        "running=$running camLive=${camSignalAgeS()} last=$lastResult appliedMs=$applied " +
    ...
    companion object {
        ...
        private const val SILENCE_PEAK = 150           // s16 peak; below = silence (liveness)
